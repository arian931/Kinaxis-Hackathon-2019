module.exports = class Enemy {
  constructor(x, y, dir, mapArray) {
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      if (this.mapArray[queue[i][1][queue[i][1].length - 1][0] + 1][queue[i][1][queue[i][1].length - 1][1]] === 0) {
        if (!vis.includes([queue[i][1][queue[i][1].length - 1][0] + 1, queue[i][1][queue[i][1].length - 1][1]].toString())) {
          const queueArrPath = JSON.parse(JSON.stringify(queue[i][1]));
          const queueArrDir = JSON.parse(JSON.stringify(queue[i][2]));
          queueArrPath.push([queue[i][1][queue[i][1].length - 1][0] + 1, queue[i][1][queue[i][1].length - 1][1]]);
          queueArrDir.push(0);
      ...
      if (this.mapArray[queue[i][1][queue[i][1].length - 1][0]][queue[i][1][queue[i][1].length - 1][1] + 1] === 0) {
        if (!vis.includes([queue[i][1][queue[i][1].length - 1][0], queue[i][1][queue[i][1].length - 1][1] + 1].toString())) {
          const queueArrPath = JSON.parse(JSON.stringify(queue[i][1]));
          const queueArrDir = JSON.parse(JSON.stringify(queue[i][2]));
          queueArrPath.push([queue[i][1][queue[i][1].length - 1][0], queue[i][1][queue[i][1].length - 1][1] + 1]);
          queueArrDir.push(1);
          queue.push([queue[i][0] + 1, queueArrPath, queueArrDir]);
        }
      }
    ...
    console.log(queue, vis);
    this.final = queue[queue.length - 1][2];
    this.cntr = 0;
  ...
  update(dt) {
    ...
    if (currentPosX === this.moveX && currentPosY === this.moveY) {
      ...
      // this.move(10, 10);
      // this.move(2, 2);
      if (this.mapArray[currentPosX + 1][currentPosY] !== 3) {
        // Right collision
        // dirs.push(0);
        // this.vis.push(currentPosX);
        // this.vis.push([currentPosX + 1, currentPosY].toString());
        // const randDir = Math.floor(Math.random() * dirs.length);
        switch (this.final[this.cntr]) {
          case 0:
